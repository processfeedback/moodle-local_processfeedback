define(['local_processfeedback/state/store', 'local_processfeedback/utils/logger'], function(__dep0, __dep1) {
const getActivityTitle = __dep0.getActivityTitle;
const getString = __dep0.getString;
const debugError = __dep1.debugError;
const debugLog = __dep1.debugLog;
const isDebugExportStepsEnabled = __dep1.isDebugExportStepsEnabled;
...
const PANEL_CLASS = 'local-processfeedback-panel';
...
const getPlacementTarget = (documentRef) => {
...
const getAssignmentEditorPlacementTarget = (documentRef) => {
    const editorContainer = documentRef.getElementById('fitem_id_onlinetext_editor');
    if (editorContainer) {
        return {
            element: editorContainer,
...
const getAssignmentStatusPlacementTarget = (documentRef) => {
    const statusTable = documentRef.querySelector('.submissionstatustable, .generaltable.submissionstatus');
    if (!statusTable) {
        return null;
    }
...
const getForumBoardPlacementTarget = (documentRef) => {
    const main = documentRef.getElementById('region-main') ||
        documentRef.querySelector('[role="main"]') ||
        documentRef.body;
    const title = [
...
const createPanel = (state, onDownload, onOpenReport, documentRef) => {
    const windowRef = documentRef.defaultView || window;
...
return { PANEL_CLASS, getPlacementTarget, getAssignmentEditorPlacementTarget, getAssignmentStatusPlacementTarget, getForumBoardPlacementTarget, createPanel };
});