...
const isProcessFeedbackPluginUrl = (link, windowRef) => {
    const parts = getPluginFileParts(link, windowRef || window);
    return parts.indexOf('assignsubmission_processfeedback') !== -1;
};
...
export const getSingleWritingProcessReportLinks = (documentRef) => {
    ...
    Array.from(documentRef.querySelectorAll('a[href]')).forEach((link) => {
        ...
        if (!isProcessFeedbackPluginUrl(link, windowRef)) {
            return;
        }
...
export const initSingleWritingProcessReportButtons = async(windowRef, documentRef, state = null) => {
    ...
    getSingleWritingProcessReportLinks(documentRef).forEach((link) => {
        ...
        if (!link.dataset.processfeedbackZip && !isProcessFeedbackPluginUrl(link, windowRef)) {
            return;
        }
