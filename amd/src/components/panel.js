...
const EXPLORE_PROCESS_URL = 'https://app.processfeedback.org/exploreprocess';
...
const createExportModal = (state, onDownload, onOpenReport, documentRef) => {
    ...
    subtitle.id = 'local-processfeedback-export-subtitle';
    subtitle.textContent = getString(state, 'exportModalSubtitle');
    overlay.setAttribute('aria-describedby', subtitle.id);
    ...
    // Focus the dialog container, not a control inside it, so assistive technology announces the
    // dialog name, its description and then the content from the top instead of jumping to a button.
    const focusDialog = () => {
        overlay.focus();
    };
