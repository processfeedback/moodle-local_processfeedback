...
final class before_standard_top_of_body_html_generation {
    ...
    public static function callback(\core\hook\output\before_standard_top_of_body_html_generation $hook): void {
        ...
        // Match mod-assign-view, mod-assign-grading, mod-forum-view, mod-forum-discuss, etc.
        // Exclude mod-assign-mod and mod-forum-mod (settings forms on modedit.php)
        // since those are handled by coursemodule_standard_elements() in lib.php.
        if (!preg_match('/^mod-(assign|forum)-(?!mod$).+$/', (string) $PAGE->pagetype, $matches)) {
            return;
        }
        $modname = $matches[1];
